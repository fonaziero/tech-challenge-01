...
const Menu = ({ left = true, textColor = 'red', backgroundColor = 'white', isMobile = true, items, isOpen, onClose }: MenuProps) => {
        ...
        <>
            <div
                className={`fixed top-0 w-64 h-full z-50 transform transition-transform duration-300 ease-in-out  ${isMoileRender}
                    ${isOpen ? (left ? 'translate-x-0' : 'translate-x-0 right-0') : (left ? '-translate-x-full' : 'translate-x-full')} bg-${backgroundColor} ${left ? 'left-0' : 'right-0'}`}
            >
                ...
                <nav className="flex flex-col space-y-4 p-4">
                    {items.map((item, index) => (
                        <a
                            ...
                            className={`border-b pb-2 ${pathname === item.link ? `text-${textColor}` : `text-${contrastColor}` }`}
                        >
                            {item.name}
                        </a>
                    ))}
